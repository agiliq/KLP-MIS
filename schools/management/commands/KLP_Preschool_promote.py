...
def move_class_sql(currentAcademicId, nextAcademicId):
    ...
    preSchooolPromoteSql = \
        """insert into schools_Student_StudentGroupRelation_2( student_id,
                student_group_id ,academic_id,active) ( select student_id ,
                student_group_id,%s,
                2 from schools_Student_StudentGroupRelation_2 where
                academic_id=%s and student_group_id in (select id from
                    schools_studentgroup_2 where institution_id in
                    (select id from  schools_institution where cat_id in
                        (11,10,12) and active=2)  and group_type='Class'))""" \
            % (nextAcademicId, currentAcademicId)

    preSchoolMoveInactivateSql = """insert into
    schools_Student_StudentGroupRelation_1( student_id,student_group_id ,
        academic_id,active) (select student_id ,
            student_group_id,academic_id,
            1 from schools_Student_StudentGroupRelation_2 where
            academic_id=%s and student_group_id in
            (select id from schools_studentgroup_2 where
                institution_id in (select id from  schools_institution
                    where cat_id in (11,10,12) and active=2)  and
                    group_type='Class'))""" \
        % (currentAcademicId)

    preSchoolDeleteSql = """delete from
    schools_Student_StudentGroupRelation_2 where academic_id=%s and
    student_group_id in (select id from schools_studentgroup_2 where
        institution_id in (select id from  schools_institution where
            cat_id in (11,10,12) and active=2)  and group_type='Class')""" \
        % (currentAcademicId)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        currentAcademicYearSql = "select id,name from schools_academic_year \
        where id=%s" % (curAcObj.id)
        ...
        nextAcademicYearSql = "select id,name from schools_academic_year \
        where name='%s'" % (nextAcademicYear)
