...
def index(fks, id):
    """Looks for the id on fks, fks is an array
    of arrays, each array has on [1] the id of the class in a dia diagram.
    When not present returns None, else
...
def dia2django(archivo):
    ...
    # diagram -> layer -> object -> UML - Class -> name, (attribs : composite
    # -> name,type)
    datos = ppal.getElementsByTagName("dia:diagram")[0].getElementsByTagName(
        "dia:layer")[0].getElementsByTagName("dia:object")
    clases = {}
    herit = []
    imports = u""
    for i in datos:
        ...
        if i.getAttribute("type") == "UML - Class":
            ...
            for j in i.childNodes:
                if j.nodeType == Node.ELEMENT_NODE and j.hasAttributes():
                    if j.getAttribute("name") == "name":
                        actclas = j.getElementsByTagName(
                            "dia:string")[0].childNodes[0].data[1:-1]
                            ...
                            if l.getAttribute("type") == "umlattribute":
                                ...
                                for k in l.getElementsByTagName("dia:attribute"):
                                    if k.getAttribute("name") == "name":
                                        nc = k.getElementsByTagName(
                                            "dia:string")[0].childNodes[0].data[1:-1]
                                    elif k.getAttribute("name") == "type":
                                        tc = k.getElementsByTagName(
                                            "dia:string")[0].childNodes[0].data[1:-1]
                                    elif k.getAttribute("name") == "value":
                                        val = k.getElementsByTagName(
                                            "dia:string")[0].childNodes[0].data[1:-1]
                                        if val == '##':
                                            val = ''
                                        ...
                                        if tc.replace(" ", "").lower().startswith("manytomanyfield("):
                                                # If we find a class not in our
                                                # model that is marked as being
                                                # to another model
                                                newc = tc.replace(
                                                    " ", "")[16:-1]
                                                if dependclasses.count(newc) == 0:
                                                        dependclasses.append(
                                                            newc)
                                ...
                                if tc.replace(" ", "").startswith("ManyToManyField("):
                                    ...
                                    if actclas == myfor:
                                        # In case of a recursive type, we use
                                        # 'self'
                                    ...
                                    tc = "models.CharField(max_length=" + varch.group(
                                        1) + ")"
                                    if len(val) > 0:
                                        tc = tc.replace(")", ", " + val + " )")
                                if not (nc == "id" and tc == "AutoField()"):
                                    clases[actclas][2] = clases[actclas][
                                        2] + ("    %s = %s\n" % (nc, tc))
            ...
            for j in a:
                if len(j.getAttribute("to")):
                    mycons[
                        int(j.getAttribute("handle"))] = j.getAttribute("to")
            print mycons
            if not 'A' in mycons:
                herit.append(mycons)
            ...
            for j in a:
                if len(j.childNodes[0].data[1:-1]):
                    imports += u"from %s.models import *" % j.childNodes[
                        0].data[1:-1]
    ...
    while i < len(ordered):
        ...
        if mark == i:
            ...
            # swap %s in %s" % ( ordered[i] , ordered[mark]) to make ordered[i]
            # to be at the end
