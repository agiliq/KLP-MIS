...
class Command(BaseCommand):
    option_list = BaseCommand.option_list + (
        ...
        make_option(
            '--format', '-f', action='store',
            dest='format', default=FORMATS[0],
            help="output format. \
            May be one of '" + "', '".join(FORMATS) + "'."),
    )
    ...
    def handle(self, *args, **options):
        ...
        if group and not Group.objects.filter(name=group).count() == 1:
            names = u"', '".join(g['name']
                                 for g in Group.objects.values('name')).encode('utf-8')
            if names:
                names = "'" + names + "'."
            raise CommandError(
                "Unknown group '" +
    ...
    def emails(self, qs, out):
        ...
        out.write(
            u",\n".join(u'%s' %
                        (ent['email']) for ent in qs).encode(self.encoding))
        out.write("\n")
    ...
    def outlook(self, qs, out):
        ...
        csvf = writer(out)
        columns = [
            'Name', 'E-mail Address', 'Notes', 'E-mail 2 Address',
            ...
            'Business Fax', 'Business Address', 'Other Phone',
    ...
    def vcard(self, qs, out):
        try:
            ...
            print self.style.ERROR("Please install python-vobject \
