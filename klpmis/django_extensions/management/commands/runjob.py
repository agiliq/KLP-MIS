...
class Command(LabelCommand):
    ...
    def runjob(self, app_name, job_name, options):
        ...
        try:
            ...
            if app_name:
                print "Error: Job %s for applabel %s\
                 not found" % (app_name, job_name)
            else:
        ...
        try:
            ...
            import traceback
            print "ERROR OCCURED IN JOB: %s \
            (APP: %s)" % (job_name, app_name)
    ...
    def handle(self, *args, **options):
        ...
        if options.get('list_jobs'):
            print_jobs(only_scheduled=False,
                       show_when=True,
                       show_appname=True)
        else:
            if not job_name:
                print "Run a single maintenance job. \
                Please specify the name of the job."
                return
