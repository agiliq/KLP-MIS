...
TODO:
 ...
 - when a table is not managed (meta.managed==False) then only do a one-way
   sqldiff ? show differences from db->table but not the other way around
...
class SQLDiff(object):
    ...
    DIFF_TEXTS = {
        ...
        'field-missing-in-db':
        "field '%(1)s' defined in model but missing in database",
        'field-missing-in-model':
        "field '%(1)s' defined in database but missing in model",
        'index-missing-in-db':
        "field '%(1)s' INDEX defined in model but missing in database",
        'index-missing-in-model':
        "field '%(1)s' INDEX defined in database schema but missing in model",
        'unique-missing-in-db':
        "field '%(1)s' UNIQUE defined in model but missing in database",
        'unique-missing-in-model':
        "field '%(1)s' UNIQUE defined in database schema but missing in model",
        'field-type-differ':
        "field '%(1)s' not of same type: db='%(3)s', model='%(2)s'",
        'field-parameter-differ':
        "field '%(1)s' parameters differ: db='%(3)s', model='%(2)s'",
        'notnull-differ':
        "field '%(1)s' null differ: db='%(3)s', model='%(2)s'",
    }

    SQL_FIELD_MISSING_IN_DB = lambda self, style, qn, args: "%s %s\n\t%s %s %s;" % (
        style.SQL_KEYWORD('ALTER TABLE'), style.SQL_TABLE(qn(args[0])), style.SQL_KEYWORD('ADD'), style.SQL_FIELD(qn(args[1])), style.SQL_COLTYPE(args[2]))
    SQL_FIELD_MISSING_IN_MODEL = lambda self, style, qn, args: "%s %s\n\t%s %s;" % (
        style.SQL_KEYWORD('ALTER TABLE'), style.SQL_TABLE(qn(args[0])), style.SQL_KEYWORD('DROP COLUMN'), style.SQL_FIELD(qn(args[1])))
    SQL_INDEX_MISSING_IN_DB = lambda self, style, qn, args: "%s %s\n\t%s %s (%s);" % (style.SQL_KEYWORD('CREATE INDEX'), style.SQL_TABLE(
        qn("%s_idx" % '_'.join(args[0:2]))), style.SQL_KEYWORD('ON'), style.SQL_TABLE(qn(args[0])), style.SQL_FIELD(qn(args[1])))
    # FIXME: need to lookup index name instead of just appending _idx to table
    # + fieldname
    SQL_INDEX_MISSING_IN_MODEL = lambda self, style, qn, args: "%s %s;" % (
        style.SQL_KEYWORD('DROP INDEX'), style.SQL_TABLE(qn("%s_idx" % '_'.join(args[0:2]))))
    SQL_UNIQUE_MISSING_IN_DB = lambda self, style, qn, args: "%s %s\n\t%s %s (%s);" % (
        style.SQL_KEYWORD('ALTER TABLE'), style.SQL_TABLE(qn(args[0])), style.SQL_KEYWORD('ADD'), style.SQL_KEYWORD('UNIQUE'), style.SQL_FIELD(qn(args[1])))
    # FIXME: need to lookup unique constraint name instead of appending _key
    # to table + fieldname
    SQL_UNIQUE_MISSING_IN_MODEL = lambda self, style, qn, args: "%s %s\n\t%s %s %s;" % (style.SQL_KEYWORD('ALTER TABLE'), style.SQL_TABLE(
        qn(args[0])), style.SQL_KEYWORD('DROP'), style.SQL_KEYWORD('CONSTRAINT'), style.SQL_TABLE(qn("%s_key" % ('_'.join(args[:2])))))
    SQL_FIELD_TYPE_DIFFER = lambda self, style, qn, args: "%s %s\n\t%s %s %s;" % (style.SQL_KEYWORD('ALTER TABLE'), style.SQL_TABLE(
        qn(args[0])), style.SQL_KEYWORD("MODIFY"), style.SQL_FIELD(qn(args[1])), style.SQL_COLTYPE(args[2]))
    SQL_FIELD_PARAMETER_DIFFER = lambda self, style, qn, args: "%s %s\n\t%s %s %s;" % (
        style.SQL_KEYWORD('ALTER TABLE'), style.SQL_TABLE(qn(args[0])), style.SQL_KEYWORD("MODIFY"), style.SQL_FIELD(qn(args[1])), style.SQL_COLTYPE(args[2]))
    SQL_NOTNULL_DIFFER = lambda self, style, qn, args: "%s %s\n\t%s %s %s %s;" % (style.SQL_KEYWORD('ALTER TABLE'), style.SQL_TABLE(
        qn(args[0])), style.SQL_KEYWORD('MODIFY'), style.SQL_FIELD(qn(args[1])), style.SQL_KEYWORD(args[2]), style.SQL_KEYWORD('NOT NULL'))
    SQL_ERROR = lambda self, style, qn, args: style.NOTICE(
        '-- Error: %s' %
        style.ERROR(args[0]))
    SQL_COMMENT = lambda self, style, qn, args: style.NOTICE(
        '-- Comment: %s' %
        style.SQL_TABLE(args[0]))
    ...
    def get_django_tables(self, only_existing):
        try:
            django_tables = self.introspection.django_table_names(
                only_existing=only_existing)
        except AttributeError:
            # backwards compatibility for before introspection refactoring
            # (r8296)
            try:
                django_tables = _sql.django_table_names(
                    only_existing=only_existing)
            except AttributeError:
                # backwards compatibility for before svn r7568
                django_tables = _sql.django_table_list(
                    only_existing=only_existing)
    ...
    def get_field_db_type(self, description, field=None, table_name=None):
        ...
        if type_code in self.DATA_TYPES_REVERSE_OVERRIDE:
            ...
            try:
                try:
                    reverse_type = self.introspection.data_types_reverse[
                        type_code]
                except AttributeError:
                    # backwards compatibility for before introspection
                    # refactoring (r8296)
                    reverse_type = self.introspection.DATA_TYPES_REVERSE.get(
                        type_code)
            except KeyError:
        ...
        if reverse_type == "DecimalField":
            ...
            kwargs['decimal_places'] = description[
                5] and abs(description[5]) or description[5]
    ...
    def find_unique_missing_in_model(self, meta, table_indexes, table_name):
        ...
        #       MySQL does
        fields = dict([(field.db_column or field.name, field.unique)
                      for field in all_local_fields(meta)])
    ...
    def find_field_missing_in_model(
            self, fieldmap, table_description, table_name):
    ...
    def find_field_type_differ(
        ...
        for field in all_local_fields(meta):
            ...
            model_type = self.strip_parameters(
                self.get_field_model_type(field))
            db_type = self.strip_parameters(
                self.get_field_db_type(description, field))

            # use callback function if defined
            if func:
                model_type, db_type = func(
                    field, description, model_type, db_type)
    ...
    def find_differences(self):
        ...
        for app_model in self.app_models:
            ...
            if cur_app_label != app_label:
                # Marker indicating start of difference scan for this
                # table_name
            ...
            table_indexes = self.introspection.get_indexes(
                self.cursor, table_name)
            fieldmap = dict([(field.db_column or field.get_attname(), field)
                            for field in all_local_fields(meta)])
            ...
            try:
                ...
                self.add_difference(
                    'error', 'unable to introspect table: %s' %
                    str(e).strip())
            ...
            self.find_unique_missing_in_model(meta,
            ...
            self.find_field_missing_in_model(
                fieldmap,
                table_description,
                table_name)
            ...
            self.find_field_parameter_differ(
                meta,
                table_description,
                table_name)
    ...
    def print_diff_text(self, style):
        ...
        for app_label, model_name, diffs in self.differences:
            ...
            if not self.dense and cur_app_label != app_label:
                print style.NOTICE(
                    "+ Application:"), style.SQL_TABLE(app_label)
            ...
            for diff in diffs:
                ...
                text = self.DIFF_TEXTS[diff_type] % dict(
                    (str(i), style.SQL_TABLE(e)) for i, e in enumerate(
                        diff_args))
                text = "'".join(
                    i % 2 == 0 and style.ERROR(e) or e for i,
                    e in enumerate(text.split("'")))
                if not self.dense:
    ...
    def print_diff_sql(self, style):
        ...
        has_differences = max(
            [len(diffs) for app_label,
             model_name,
             diffs in self.differences])
        if not has_differences:
            ...
            for app_label, model_name, diffs in self.differences:
                ...
                if not self.dense and cur_app_label != app_label:
                    print style.NOTICE(
                        "-- Application: %s" % style.SQL_TABLE(app_label))
                    cur_app_label = app_label
                if not self.dense:
                    print style.NOTICE(
                        "-- Model: %s" % style.SQL_TABLE(model_name))
                for diff in diffs:
                    diff_type, diff_args = diff
...
class MySQLDiff(SQLDiff):
    ...
    def get_field_db_type(self, description, field=None, table_name=None):
        ...
        # weird bug? in mysql db-api
            # where it returns three times the correct value for field length
        ...
        # exclude tinyints from conversion.
        ...
        if field:
            ...
            if db_type == 'integer' and description[1] == FIELD_TYPE.SHORT:
                # FIXME: what about if it's not UNSIGNED ?
...
class SqliteSQLDiff(SQLDiff):
    ...
    # Finding Indexes by using the get_indexes dictionary
    # doesn't seem to work for sqlite.
    def find_index_missing_in_db(self, meta, table_indexes, table_name):
        pass
...
class PostgresqlSQLDiff(SQLDiff):
    ...
    SELECT nspname, relname,
        conname, attname, pg_get_constraintdef(pg_constraint.oid)
    FROM pg_constraint
    INNER JOIN pg_attribute ON pg_constraint.conrelid =
        pg_attribute.attrelid AND pg_attribute.attnum = any(
            pg_constraint.conkey)
    INNER JOIN pg_class ON conrelid=pg_class.oid
    INNER JOIN pg_namespace ON pg_namespace.oid=pg_class.relnamespace
    ORDER BY CASE WHEN contype='f' THEN 0 ELSE 1 END,
        contype,nspname,relname,conname;
    """
    SQL_LOAD_NULL = """
    ...
    SQL_FIELD_TYPE_DIFFER = lambda self, style, qn, args: "%s %s\n\t%s %s %s %s;" % (style.SQL_KEYWORD('ALTER TABLE'), style.SQL_TABLE(
        qn(args[0])), style.SQL_KEYWORD('ALTER'), style.SQL_FIELD(qn(args[1])), style.SQL_KEYWORD("TYPE"), style.SQL_COLTYPE(args[2]))
    SQL_FIELD_PARAMETER_DIFFER = lambda self, style, qn, args: "%s %s\n\t%s %s %s %s;" % (style.SQL_KEYWORD('ALTER TABLE'), style.SQL_TABLE(
        qn(args[0])), style.SQL_KEYWORD('ALTER'), style.SQL_FIELD(qn(args[1])), style.SQL_KEYWORD("TYPE"), style.SQL_COLTYPE(args[2]))
    SQL_NOTNULL_DIFFER = lambda self, style, qn, args: "%s %s\n\t%s %s %s %s;" % (style.SQL_KEYWORD('ALTER TABLE'), style.SQL_TABLE(
        qn(args[0])), style.SQL_KEYWORD('ALTER COLUMN'), style.SQL_FIELD(qn(args[1])), style.SQL_KEYWORD(args[2]), style.SQL_KEYWORD('NOT NULL'))
    ...
    def get_field_db_type(self, description, field=None, table_name=None):
        db_type = super(
            PostgresqlSQLDiff, self).get_field_db_type(description)
        if not db_type:
            return
        if field:
            ...
            if table_name:
                ...
                check_constraint = self.check_constraints.get(
                    (tablespace,
                     table_name,
                     field.attname),
                    {}).get('pg_get_constraintdef',
                            None)
                if check_constraint:
                    check_constraint = check_constraint.replace("((", "(")
                    check_constraint = check_constraint.replace("))", ")")
                    check_constraint = '("'.join([')' in e and '" '.join(
                        e.split(" ", 1)) or e for e in check_constraint.split("(")])
    ...
    def find_field_type_differ(self, meta, table_description, table_name):
        ...
        super(PostgresqlSQLDiff, self).find_field_type_differ(meta,
            table_description, table_name, callback)
    """
...
class Command(BaseCommand):
    option_list = BaseCommand.option_list + (
        make_option(
            ...
            help="Automaticly include all \
            application from INSTALLED_APPS."),
        make_option(
            '--not-only-existing', '-e', action='store_false',
            dest='only_existing',
            help="Check all tables that exist in the database,not \
            only tables that should exist based on models."),
        make_option(
            '--dense-output', '-d', action='store_true',
            dest='dense_output',
            help="Shows the output in dense format, normally \
            output is spreaded over multiple lines."),
        make_option(
            '--output_text', '-t', action='store_false',
            dest='sql', default=True,
            help="Outputs the differences as \
            descriptive text instead of SQL"),
    )

    help = """Prints the (approximated) difference between models \
    and fields in the database for the given app name(s).

It indicates how columns in the database are different from the sql
that would be generated by Django. This command is not a
database migration tool. (Thoughit can certainly help) It's purpose
is to show the current differences as a way
to check/debug ur models compared to the real database tables and columns."""
    ...
    def handle(self, *app_labels, **options):
        ...
        if engine == 'dummy':
            ...
            raise CommandError(
                """Django doesn't know which syntax to use for your
                 SQL statements,because you haven't specified the
                 DATABASE_ENGINE setting.Edit your settings file and
                 change DATABASE_ENGINE to something like
                 'postgresql' or 'mysql'.""")
