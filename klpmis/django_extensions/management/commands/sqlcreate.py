...
class Command(BaseCommand):
    option_list = BaseCommand.option_list + (
        make_option('-R', '--router', action='store',
                    ...
                    help='Use this router-database other \
                    then defined in settings.py'),
        make_option('-D', '--drop', action='store_true',
                    dest='drop', default=False,
                    help='If given, includes commands to drop any existing \
                    user and database.'),
    )
    help = """Generates the SQL to create your database for you, as \
    ...
    def handle(self, *args, **options):
        ...
        if django.get_version() >= "1.2":
            ...
            if not got_db_settings:
                raise CommandError(
                    "You are using Django %s which requires to\
                     specify the db-router.\nPlease specify the \
                     router by adding --router=<routername> to \
                     this command." %
                    django.get_version())

        # print "%s %s %s %s" % (settings.DATABASE_ENGINE,
        ...
        # django settings file tells you that localhost
        # should be specified by leaving
        ...
        if engine == 'mysql':
            ...
            -- Please read this carefully! Collation will
            be set to utf8_bin to have case-sensitive data.""")
            print "CREATE DATABASE %s CHARACTER SET utf8 COLLATE utf8_bin;" % dbname
            print "GRANT ALL PRIVILEGES \
            ON %s.* to '%s'@'%s' identified by '%s';" % (
            ...
            print "CREATE USER %s WITH ENCRYPTED \
            PASSWORD '%s' CREATEDB;" % (dbuser, dbpass)
            print "CREATE DATABASE %s WITH ENCODING \
            'UTF-8' OWNER \"%s\";" % (dbname, dbuser)
            print "GRANT ALL PRIVILEGES ON DATABASE %s TO %s;" % (dbname, dbuser)
        elif engine == 'sqlite3':
            sys.stderr.write(
                "-- manage.py syncdb will automatically \
            ...
            print "GRANT ALL PRIVILEGES ON \
            DATABASE %s to %s" % (dbname, dbuser)
