...
def generate_dot(app_labels, **kwargs):
    ...
    for app in apps:
        ...
        for appmodel in appmodels:
            abstract_models = abstract_models + [
                abstract_model for abstract_model in appmodel.__bases__ if hasattr(
                    abstract_model,
                    '_meta') and abstract_model._meta.abstract]
        ...
        for appmodel in appmodels:
            ...
            # FIXME: actually many_to_many fields aren't
                # saved in this model's db table, so why should we
                    # add an attribute-line for them in the resulting graph?
            ...
            for field in appmodel._meta.local_many_to_many:
                ...
                if isinstance(field, ManyToManyField):
                    ...
                    add_relation(
                        field,
                        mark_safe('[style="dotted", arrowhead=normal, \
                            arrowtail=normal, dir=both]'))
...
def main():
    try:
        opts, args = getopt.getopt(
            sys.argv[
                1:], "hadgi:L:x:X:en", ["help", "all_applications",
                                        "disable_fields",
                                        "group_models",
                                        "include_models=",
                                        "inheritance",
