    ...
    for app_mod in get_apps():
        ...
        for model in app_models:
            try:
                ...
                if not quiet_load:
                    print style.ERROR(
                        "Failed to import '%s' from '%s' reason: %s" % (model.__name__, app_name, str(e)))
                continue
        if not quiet_load:
            print style.SQL_COLTYPE(
                "From '%s' autoload: %s" % (app_mod.__name__.split('.')[-2], ", ".join(model_labels)))
