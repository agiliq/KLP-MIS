...
class ActivatorModelManager(QuerySetManager):
    ...
    def active(self):
        """ Returns active instances of ActivatorModel:
            SomeModel.objects.active() """
        return (
            super(ActivatorModelManager, self).get_query_set().filter(
                status=1)
        )

    def inactive(self):
        """ Returns inactive instances of ActivatorModel:
...
class ActivatorModel(Document):
    ...
    """ ActivatorModel
    An abstract base class model that provides activate and \
    deactivate fields.
    """
    STATUS_CHOICES = (
        (0, _('Inactive')),
