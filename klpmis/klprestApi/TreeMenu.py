...
def hasChild(query, typ, boundaryType, filterBy, secFilter,
             permFilter, assessmentPerm, shPerm, userSel,
             request, showflag):
...
def KLP_assignedInstitutions(userId):
    ...
    rawQuerySet = \
        Institution.objects.raw(""" SELECT "id","obj_id" FROM
            "public"."object_permissions_institution_perms" WHERE
                "user_id" = '%s' AND "Acess" = 1 """ % userId)
    inst_list = []
...
def KLP_assignedAssessmentInst(userId, assessmentId):
    ...
    inst_list = \
        UserAssessmentPermissions.objects.filter(
            user__id=userId, assessment__id__in=assessmentId,
            access=True).defer('user', 'assessment',
                               'instituion'
                               ).values_list('instituion__id',
                                             flat=True).distinct()
    return inst_list
...
def tree(request):
    ...
    if typ == 'source':
        ...
        # if home is true query for boundaries........
            ...
            # if logged in user is super user or staff or in AdminGroup and
            # filterBy is none query all active boundary's where parent is 1
            # and based on boundaryType

                query = Boundary.objects.filter(
                    parent__id=1, active=2,
                    boundary_type=boundaryType).defer('boundary').order_by(
                    'name').extra(select={'lower_name':
                                          'lower(name)'
                                          }).order_by('lower_name')
            else:
                if logUser.is_superuser or logUser.is_staff \
                ...
                # if logged in user is super user or staff or in AdminGroup and
                # filterBy is not none query all active SG's based on
                # assessments
                    ...
                    map_institutions_list = getAssInst(
                        secFilter)
                ...
                try:
                    boundary_list = \
                        Boundary.objects.filter(
                            ...
                            active=2,
                            boundary_type=boundaryType).defer(
                            'boundary').values_list('parent__parent__id',
                                                    flat=True).distinct()
                except:
                    boundary_list = \
                        Boundary.objects.filter(
                            ...
                            active=2,
                            boundary_type=boundaryType).defer(
                            'boundary').values_list('parent__id',
                                                    flat=True).distinct()

                query = Boundary.objects.filter(
                    pk__in=boundary_list,
                    active=2,
                    parent__id=1).defer('boundary').distinct().extra(
                    select={'lower_name':
                            'lower(name)'}).order_by('lower_name')
        else:
        ...
        # if data is not true query for all active programmes.
            ...
            query = \
                Programme.objects.filter(
                    active=2,
                    programme_institution_category=boundaryType).extra(
                    select={'lower_name':
                            'lower(name)'}).order_by('-start_date',
                                                     '-end_date',
                                                     'lower_name')
            typ = 'programme'
    else:
        ...
        # if typ is boundary Query for sub boundaries or Institutions
            ...
            # and based on boundaryType
                ...
                query = \
                    Boundary.objects.filter(
                        ...
                        active=2,
                        boundary_type=
                        boundaryType).defer('boundary').extra(select={
                        'lower_name':
                        'lower(name)'}).order_by('lower_name')
            else:
                if logUser.is_superuser or logUser.is_staff \
                ...
                # if user is not superuser and not staff and not related to
                # admin group and filterby is none get all assigned
                # institutions.
                    ...
                    map_institutions_list = getAssInst(secFilter)
                    institutions_list = list(set(map_institutions_list)
                                             & set(
                                                 KLP_assignedAssessmentInst(
                                                     logUser.id,
                ...
                # Query for Boundaries based on institutions
                    ...
                    boundaries = \
                        Boundary.objects.filter(
                            ...
                            active=2,
                            boundary_type=
                            boundaryType).defer('boundary').values_list(
                            'id', flat=True).distinct()
                    query = \
                        Boundary.objects.filter(
                            id__in=boundaries).defer(
                            'boundary').extra(select={
                            'lower_name': 'lower(name)'}).order_by(
                            'lower_name')
            ...
            # If Query is Empty Query for Institutions under boundary
                ...
                # if logged in user is super user or staff or in AdminGroup and
                # filterBy is none query all active institutions's based on
                # boundary
                    ...
                    # in AdminGroup and filterBy is not none
                    # query all active SG's based on assessments
                    # studentgroup_list =
                    #   Assessment_StudentGroup_Association.objects.filter(
        ...
        # if typ is programme Query For
        # active assessment based On programme id
        ...
        # if typ is assessment Query For active Questions based On assessment
        # id
    ...
    CDict = hasChild(query, typ, boundaryType, filterBy, secFilter,
                     permFilter, assessmentPerm, shPerm, userSel, request,
                     showflag)
...
def getAssInst(secFilter, superuser=1):
    ...
    for k in secFilter:
        ...
        studentgroup_list = \
            asstable[asstype].objects.filter(
                assessment=assobj,
    ...
    for k in secFilter:
        ...
        if asstype in [2, 3]:
            studentgroup_list = \
                asstable[asstype].objects.filter(
                    assessment=assobj,
                    student_group__id__in=sg).values_list(
                    'student_group__id', flat=True).distinct()
            sg_list = \
                StudentGroup.objects.filter(
                    id__in=studentgroup_list,
                    active=2).defer('institution').values_list(
                    'id', flat=True).distinct()
        else:
            if sg:
                sg_list = [sg[0]]
