...
class KLP_ChangeAns(Resource):
    ...
    def read(self, request):
        ...
        # get Assessment Id
        assessmentId = int(request.POST.get('assessmentId'))

        # assessmentObj1=Assessment.objects.filter(
            # pk=assessmentId).defer("programme")
        ...
        student_groupObj = \
            StudentGroup.objects.filter(
                pk=student_groupId).defer('institution')

        if assessmentObj.typ == 3:
            modelName = 'student'
            # get SG Object based on id
            student_groupObj = student_groupObj.values('institution')[0]
            instObj = \
                Institution.objects.filter(
                    pk=student_groupObj[
                        'institution']).defer('boundary',
                                              'cat', 'mgmt',
                                              'inst_address')[0]

            studentObj1 = Student.objects.filter(
                id=student.split('_')[0]).defer('child')
           ...
           # childObj=studentObj1.defer("child")[0]
            ...
            instObj = \
                Institution.objects.filter(
                    pk=student_groupId).defer('boundary', 'cat',
                                              'mgmt', 'inst_address')
            studentObj = instObj[0]
        Questions_list = \
            Question.objects.filter(
                assessment__id=assessmentId)  # get questions under assessment
        ...
        for (index, question) in enumerate(Questions_list):
            ...
            ansIdfieldVal = 0 if request.POST.get(ansIdfield, '') in [
                None, 'None'] else request.POST.get(ansIdfield, '')
            ...
            if ansObj:
                ...
                if textFieldVal:
                    if textFieldVal.lower() == 'ab':
                        ...
                        # If text field value is ab(absent) then change
                        # answer_grade and answer_score to none and status to
                        # -99999
                        ...
                        # If text field value is uk(unknown) then change
                        # answer_grade and answer_score to none and status to
                        # -1
                        ...
                        # else question type is 2(Grade) then change status to
                        # none and store textfield value in answer_grade
                        ...
                        # else question type is 1(Marks) then change status to
                        # none and store textfield value in answer_score
                    ...
                    if ansObj.double_entry == 1 and user1id == userid:
                        ...
                        # if the double_entry value for answer is 1(only first
                        # user enter data) and user1 is same as logged in user
                        # change last_modified_by to current user

                        ansObj.last_modified_by = user1
                    else:

                    # else update double_entry to 2(second user also submits
                    # data), last_modified_by and user2 to logged user
                    ...
                    try:
                        ...
                        newanswerdata[
                            'form-0-answer_score'] = ansObj.answer_score
                        newanswerdata[
                            'form-0-last_modified_by'] = ansObj.last_modified_by.id
                        # newanswerdata['form-0-answerText']=ansObj.answerText
                        newanswerdata[
                            'form-0-double_entry'] = ansObj.double_entry
                        newanswerdata['form-0-status'] = ansObj.status
                        newanswerdata[
                            'form-0-answer_grade'] = ansObj.answer_grade
                        ...
                        firstuserdata1 = \
                            Answer.objects.filter(question__id=
                                                  question.id,
                                                  user1__id=user.id,
                                                  object_id=studentObj.id,
                                                  id=ansObj.id)
                        ...
                        if not firstuserdata1.count() == 0:
                            ...
                            if ansForm.errors:
                                if '__all__' in ansForm.errors[0]:
                                    return (
                                        'Primary Value is already existing  '
                                    )
                            ansForm.save()
                ...
                if textFieldVal:
                    ...
                    (status, answer_grade, answer_score) = (
                        None, None, None)
                    if textFieldVal.lower() == 'ab':

                    # If text field value is ab(absent) then set status to
                    # -99999
                    ...
                    # else if  question type is 2(Grade) then store textfield
                    # value in answer_grade
                        ...
                        # else if  question type is 1(Marks) then store
                        # textfield value in answer_score
                    ...
                    # ansObj = Answer(question=question, content_type = cTObj,
                    ...
                    try:
                        ...
                        firstuserdata = \
                            Answer.objects.filter(question__id=question.id,
                                                  user1__id=user.id,
                                                  object_id=studentObj.id,
                                                  flexi_data=primaryfieldVal)
                        ...
                        try:
                            fdata = Answer.objects.get(question__id=
                                                       question.id,
                                                       object_id=
                                                       studentObj.id,
                                                       flexi_data=
            ...
            try:
                ...
                pass
        return (
            # ,question.id,',',studentObj.id,',',dE,',
            # ',status,',',answer_grade,answer_score,user.id
            'Data(s) Saved |' + ','.join(ansIds)
        )
...
def KLP_DataValidation(request):
    ...
    #  Get ValidateId (student and question id)
    validateId = request.POST.get('validateField')
    #  Get Text field value to validate
        ...
        if dEntry in [0, 2]:
            ...
            if validateValue:
                if validateValue.lower() == 'ab':
                    ...
                    # If text field value is ab(absent) and answer status is
                    # -99999 then return true

                    if ansObj.status == -99999:
                        respStr = True
                elif validateValue.lower() == 'uk':

                    # If text field value is uk(unknown) and answer status is
                    # -1 then return true
                    ...
                    if 1:
                            ...
                            # If question type is 2(Grade) then match with
                            # answer grade if matches return true
                    ...
                    if 1:
                            ...
                            # If question type is 1(marks) then match with
                            # answer score if matches return true
