...
def KLP_StudentGroup_Create(request, referKey):
    ...
    KLP_Create_StudentGroup = KLP_StudentGroup(
        queryset=StudentGroup.objects.filter(pk=0),
        permitted_methods=('GET',
                           'POST'),
        responder = TemplateResponder(
            template_dir='viewtemplates',
            template_object_name='studentgroup',
            extra_context={
                'buttonType': buttonType,
                'ParentKey': referKey,
                'group_typ': group_typ,
                'sch_typ':
                instObj.boundary.boundary_category.boundary_category}),
...
def KLP_StudentGroup_View(request, studentgroup_id):
    ...
    studentGroups = StudentGroup.objects.filter(
        institution__id=studgrpParent.id,
        group_type="Center",
        active=2)
    ...
    child_list = Student_StudentGroupRelation.objects.filter(
        ...
        academic=current_academic,
        active=2,
        student__active=2).values_list('student__child',
                                       flat=True)

    students = Child.objects.filter(id__in=child_list).extra(
        select={
            'lower_firstname': 'lower(trim("first_name"))',
            'lower_midname': 'lower(trim("middle_name"))',
            'lower_lastname': 'lower(trim("last_name"))'}
    ).order_by('lower_firstname',
               'lower_midname',
               'lower_lastname')
    resp = Collection(
        students,
...
def KLP_StudentGroup_Update(request, studentgroup_id):
    ...
    # before StudentGroup.objects.all()
    KLP_Edit_StudentGroup = KLP_StudentGroup(
        queryset=StudentGroup.objects.filter(pk=studentgroup_id),
        permitted_methods=('GET',
                           'POST'),
        responder = TemplateResponder(template_dir='edittemplates',
                                      template_object_name='studentgroup',
                                      extra_context={'buttonType':
                                                     buttonType,
                                                     'ParentKey': ParentKey,
                                                     'group_typ': group_typ,
                                                     'sch_typ': sch_typ}),
...
def KLP_StudentGroup_Answer_Entry(
        request, studentgroup_id, programme_id, assessment_id):
    """ To Show Answer Entry Form
        studentgroup/(?P<studentgroup_id>\d+)/programme/(?P<programme_id>\d+)/
        assessment/(?P<assessment_id>\d+)/view/"""
    """ This Method is used for to generate student answers
            grid to enter data/answers for the assessment questions """
    ...
    if AssObj.typ == 3:
        ...
        grupObj = StudentGroup.objects.filter(
            pk=studentgroup_id).only("group_type")[0].group_type
        childs_list = \
            Child.objects.filter(id__in=students).extra(select={
                'lower_firstname':
    ...
    question_list = Question.objects.filter(
        ...
        active=2).defer("assessment").distinct()

    ansObjs = Answer.objects.filter(id=0)
    studIdList, qNamesList, qIdList, chList, rDict, childDict, counter = [
    ], [], [], [], {}, {}, 0
    ...
    if AssObj.flexi_assessment:
        ansflexObj = list(
            set(Answer.objects.filter(
                question__in=question_list,
                object_id__in=studIdList
                ).distinct().order_by('id').values_list('flexi_data',
                                                        flat=True)))
        ansflexObj.sort()
    else:
        ansflexObj = Answer.objects.filter(
            question__in=question_list,
            object_id__in=studIdList
        ).order_by('id').values_list('flexi_data', flat=True)
    if ansflexObj:
    ...
    for qindex, ques in enumerate(question_list):
        ...
        if qType == 2:
            # if quetion type is 2(marks) get grades to do validation while
            # data entry
            ansIn = ques.grade
            dataDict['ansIn'] = ansIn
        else:
            # else get minmum and maximum score to do validation while data
            # entry
    ...
    if not AssObj.flexi_assessment:
        ...
        if AssObj.primary_field_type == 4:
            lookupfields = Assessment_Lookup.objects.filter(
                assessment=AssObj).order_by('rank', 'name')
    ...
    for indi, flexidata in enumerate(ansflexObj):
        ...
        for qindex, ques in enumerate(question_list):
            ...
            if not AssObj.flexi_assessment:
                ...
                ansObjs = ansObj = Answer.objects.select_related(
                    "user1").filter(question__id=qId,
                                    object_id=studentId)
            else:
                ansObjs = ansObj = Answer.objects.select_related(
                    "user1").filter(question__id=qId,
                                    object_id__in=studIdList,
                                    flexi_data=flexidata)
            ...
            if ansObj:
                for index, ansObj in enumerate(ansList):
                    ...
                    if dEntry == 2:
                # if dEntry is 2 (doubleentry is finished) then dont show input
                # box
                        ansDict['iBox'] = False
                    else:
                        # else show input box
                ...
                # (refer ticket 322)x
                    ...
                    if status == -99999:
                        ...
                        # if answer status is -1(unknown) then show answer
                        # value as 'UK'.
                        ansVal = 'UK'
                    elif qType == 2:
                    ...
                    if firstUser != user.id and dEntry == 1:
                        ...
                        # not finished) and logged in user is not match with
                        ...
                        # to show answer value in input box.
                        ansDict['shVal'] = True
    ...
    if AssObj.flexi_assessment:
        ...
        for k in range(1, (ordercounter - lenansflexObj) + 1):
            ...
            for qindex, ques in enumerate(question_list):
                ...
                studIdListprifix.append(
                    str(studentId) + '_' + str(k + lenansflexObj))

                rDict[str(qId) + '_' + str(studentId) +
                      '_' + str(k + lenansflexObj)] = ansDict
    ...
    val = Collection(
        ...
        responder = TemplateResponder(
            ...
            extra_context={
                ...
                'group_typ': grupObj, 'url': url,
                'studIdList': studIdList,
                'qNamesList': qNamesList, 'chList': chList,
                'childDict': childDict, 'rDict': rDict,
                'qIdList': qIdList, 'AssObj': AssObj,
                'loguser': request.user.id,
                'lookupfields': lookupfields,
                'studIdListprifix': noAnsListprefix,
                'ordercounter': ordercounter,
                'formmanage': form.management_form}),
