...
def KLP_Question_Create(request, referKey):
    """ To Create New Question
            assessment/question/(?P<referKey>\d+)/creator/"""
    ...
    response = \
        KLP_Create_Question.responder.create_form(request,
...
def KLP_Question_Update(request, question_id):
    """ To update Selected Question
            question/(?P<question_id>\d+)/update/"""
    ...
    KLP_Edit_Question = \
        KLP_Question(queryset=Question.objects.filter(pk=question_id),
                     ...
                     receiver=XMLReceiver())
    response = \
        KLP_Edit_Question.responder.update_form(request,
                                                pk=question_id,
                                                form_class=Question_Form)
