...
def KLP_Assessment_View(request, assessment_id):
    ...
    copyEnable = True if (not assessment_lookup
                          and assssment_obj[0].primary_field_type == 4
                          and assessment_obj[0].flexi_assessment
                         ) else False
                         # before Assessment.objects.all()

    template_responder = TemplateResponder(template_dir='viewtemplates',
                                           template_object_name='assessment',
                                           extra_context={'copyEnable':
                                                          copyEnable})
    resp = KLP_Assessment(queryset=assObj,
                          permitted_methods=('GET', 'POST'),
                          responder=template_responder,)(request, assessment_id, **kwrg)
