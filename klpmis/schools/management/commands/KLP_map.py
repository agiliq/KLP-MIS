...
class Command(BaseCommand):
    ...
    ''' Command To map assessments with student group and to
    assign permissions to users automatically. And then it
    list out the user permissions.'''
        ...
        if 1:
            ...
            if fileName and assessment_id:
                if 1:
                    ...
                    #  splits student group ids by ,
                    sgList = studenGroups.split(',')
                    assessmentObj = \
                        Assessment.objects.filter(id=assessment_id).defer(
                            'programme')[0]

                    # get assessment list under programme to assign permissions
                    # to user.
                    ...
                    assessment_list = \
                        Assessment.objects.filter(
                            programme=prgObj,
                            active=2).defer('programme').values_list('id',
                                                                     flat=True
                                                                     ).distinct()
                    inst_list = []
                    ...
                    for sg in sgList:
                        ...
                        if sg:
                            ...
                            if sgObj:
                                AssStudAssForm = modelformset_factory(
                                    Assessment_StudentGroup_Association,
                                    form=Assessment_StudentGroup_Association_Form)
                                ...
                                if assessmentObj.typ == 3:
                                    # mapping assesment and
                                    # student group
                                    ASAObj = Assessment_StudentGroup_Association.objects.filter(
                                        student_group=sgObj, assessment=assessmentObj)
                                    # mapObj =
                                    # \Assessment_StudentGroup_Association
                                    #   (assessment=assessmentObj,student_group=sgObj,
                                    # active=2)
                                    AssStudAssForm = modelformset_factory(
                                        Assessment_StudentGroup_Association,
                                        form=Assessment_StudentGroup_Association_Form)
                                    MappingStr = \
                                        ...
                                        % (assessmentObj.name, sgObj.name, sgObj.section)
                                                                ...
                                                                # assesment and
                                                                # student group
                                    ASAObj = Assessment_Class_Association.objects.filter(
                                        student_group=sgObj, assessment=assessmentObj)
                                    # mapObj =
                                    ...
                                    AssStudAssForm = modelformset_factory(
                                        Assessment_Class_Association,
                                        form=Assessment_Class_Association_Form)
                                    MappingStr = \
                                        '%s - Assessment and StudentGroup - %s%s ...\n' \
                                        % (assessmentObj.name, sgObj.name, sgObj.section)
                                else:

                                    # mapping assement Institution
                                    ASAObj = Assessment_Institution_Association.objects.filter(
                                        institution=sgObj, assessment=assessmentObj)
                                    # mapObj =
                                    # \Assessment_Institution_Association(
                                      # assessment=assessmentObj,
                                      # institution=sgObj,active=2)
                                    AssStudAssForm = modelformset_factory(
                                        Assessment_Institution_Association,
                                        form=Assessment_Institution_Association_Form)
                                    MappingStr = \
                                ...
                                if ASAObj:
                                    ...
                                    '''
                                    self.stdout.write(
                                        '%s are Already Mapped ...\n' %
                                        MappingStr)
                                else:
                                    requestcopy['form-INITIAL_FORMS'] = 0
                                    rform = AssStudAssForm(
                                        requestcopy,
                                        requestcopy)

                                    if '__all__' in rform.errors[0]:
                                        self.stdout.write('%s are Already Mapped ...\n'
                                                          % MappingStr)
                                    else:
                                        try:
                                            ascopy = Assessment_Institution_Association.objects.get(
                                                institution__id=requestcopy[
                                                    'form-0-student_group'],
                                                assessment__id=requestcopy['form-0-assessment'])
                                        except:
                                            if not assessmentObj.typ == 1:
                                                rform.save()
                                            else:
                                                if not Assessment_Institution_Association.objects.filter(
                                                        assessment__id=requestcopy['form-0-assessment'],
                                                        institution__id=requestcopy['form-0-assessment']).exists():
                                                    ...
                                                    and institution_id = %d and active=2 """ % (
                                                        requestcopy['form-0-assessment'],
                                                        requestcopy['form-0-student_group'])
                                                    cursor.execute(q1)
                                                    res = cursor.fetchone()
                                                    bt = (0,)
                                                    qres = cmpT(res, bt)
                                                    if qres:
                                                        qu = """insert into
                                                        schools_Assessment_Institution_Association(
                                                          assessment_id, institution_id, active) \
                                                             values (%s, %s, 2) """ % (requestcopy['form-0-assessment'],
                                                                                       requestcopy['form-0-student_group'])
                                                        cursor.execute(qu)
                                                    else:
                                                        print "record exist"
                    ...
                    for user in users_List:
                        ...
                        # get institutions assigned to
                        # user to generate report and
                        # to verify user permission.
                        ...
                        perm_instSet = \
                            list(
                                set(inst_list).intersection(set(perm_instList)))
                        InsObjs = \
                            Institution.objects.filter(
                                id__in=perm_instSet).defer('boundary',
                                                           'cat', 'mgmt',
                                                           'inst_address')
                        ...
                        # if users has permission for all assessments
                        # in programme check user has permission
                        # with instituion.
                        # if user has permission then assign assessment
                        # permission

                            if reportflag:
                                permInstObj = instObj

                            # To generate Institution permission
                            # report
                        ...
                        if reportflag:
                            ...
                            for asmPermObj in asmPermObjs:
                                ...
                                asmPermData.append(
                                    asmPermObj.assessment.programme)
                        ...
                        self.stdout.write('Total time was taken for this user %s'
                                          % str(lastprocestime))
                    self.stdout.write('''Total time was taken for all the users %s'''
                                      % str(datetime.datetime.now() - starttime))
                else:

                         # except IOError:
                    # If Arguments are not in proper order raises an command
                    # error

                    raise CommandError('Pass First Parameter is \
                      FileName and Second Parameter is Assessment Id\n'
                                       )
        else:
