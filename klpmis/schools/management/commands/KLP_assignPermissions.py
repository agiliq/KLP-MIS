...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if 1:
            ...
            # asmIdList = assignPermission(inst_list, deUserList,
                # permissions, permissionType, assessmentId, assessmentPerm)

                if bound_cat == 'district':
                    inst_listall = \
                        Institution.objects.filter(
                            boundary__parent__id__in=bound_list,
                            active=2).values_list('id',
                                                  flat=True).distinct()
                elif bound_cat in ['block', 'project']:
                    inst_listall = \
                        Institution.objects.filter(
                            boundary__id__in=bound_list,
                            active=2).values_list('id',
                                                  flat=True).distinct()
        ...
        if 0:
            raise CommandError('Pass First Parameter as Boundary Ids List \
                file and Second Parameter as User Ids List \n'
                               )
    ...
    def SendingMail(
        ...
        detailmessage = \
            'The following is the Assignment data for the\
             current action only:\n'
        for k in dic:

            detailmessage += '''User name :%s (%d)''' % (k[0], k[1])
            assIds = dic[k][0]
            for assId in assIds:
                assName = Assessment.objects.filter(id=assId)[0].name
                permission_update_status()
                detailmessage += \
                    ''' \t \t Permissions were already assigned to the \
                    following institutions for assessment %s :''' \
                    % (str(assId) + '-' + assName)
                detailmessage += permission_status_message(assIds, assId, 0)

                detailmessage += \
                    ''' \t \t Permissions were newly
                    assigned to the following \
                    institutions for assessment %s :''' \
                    % (str(assId) + '-' + assName)
            ...
            if dic[k][1] or dic[k][2]:
                detailmessage += \
                    ''' \t \t Permissions were newly assigned
        ...
        subject = \
            'Assigned Permissions for Institutions and \
            Assessments by %s from %s server' \
        ...
        assessmentPermtext = ('True' if assessmentPerm not in ['None',
                              None, ''] else 'False')
        fullmsg = \
            '''%s%s User List : %s Admin Action :%s Assessment Id List : %s
