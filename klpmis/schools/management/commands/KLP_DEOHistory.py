            ...
            if fileName and start_date and end_date:
                try:
                    ...
                    if not os.path.exists(path):
                                            ...
                                            # if dir not exists creates
                                            # directory with name logfiles in
                                            # cwd.
                    ...
                    headerList = [
                        'Sl.No', 'User', 'pre_boundary_created',
                        'pre_boundary_mod', 'pre_boundary_del',
                        'primary_boundary_created',
                        'primary_boundary_mod',
                        'primary_boundary_del',
                        'pre_sch_created', 'pre_sch_mod',
                        'pre_sch_del', 'primary_sch_created',
                        'primary_sch_mod', 'primary_sch_del',
                        'pre_stud_created', 'pre_stud_mod',
                        'pre_stud_del', 'primary_stud_created',
                        'primary_stud_mod', 'primary_stud_del',
                        'pre_teacher_created', 'pre_teacher_mod',
                        'pre_teacher_del', 'primary_teacher_created',
                        'primary_teacher_mod', 'primary_teacher_del',
                    ]
                    (asmDict, asmList) = ({}, [])
                    # .filter(username__iexact="nalini").order_by("username").only("id", "username")
                    ...
                    for assessment in assessments:
                        ...
                        answers = \
                            Answer.objects.filter(Q(user1__id__in=userIds)
                                                  ...
                                                  last_modified_date__range=(
                                                      sDate, eDate),
                                                  question__assessment=assessment).values_list('id', flat=True).distinct()
                        if answers:
                        ...
                        # get the content objects(instituion, staff, student)
                            ...
                            preSchList = \
                                Institution.objects.filter(id__in=inst_list,
                                                           boundary__boundary_type__id=2).values_list('id', flat=True)
                            primarySchList = \
                                Institution.objects.filter(id__in=inst_list,
                                                           boundary__boundary_type__id=1).values_list('id', flat=True)
                            ...
                            for content in contentList:
                                ...
                                if content == 'boundary':
                                    (preBoundaryList,
                                     primaryBoundaryList) = ([], [])
                                    BoundaryList = institution_list_query(
                                        preSchList, 'boundary')
                                    ...
                                    BoundaryList = boundary_list_query(
                                        preBoundaryList, 1, 'parent')
                                    preBoundaryList.extend(list(BoundaryList))

                                    BoundaryList = institution_list_query(
                                        primarySchList, 'boundary')
                                    primaryBoundaryList.extend(
                                        list(BoundaryList))

                                    BoundaryList = boundary_list_query(
                                        primaryBoundaryList, 1, 'parent')
                                    primaryBoundaryList.extend(
                                        list(BoundaryList))

                                    BoundaryList = boundary_list_query(
                                        primaryBoundaryList, 2, 'parent')
                                    primaryBoundaryList.extend(
                                        list(BoundaryList))

                                    # ['%s' %i for i in preBoundaryList]
                                    preList = preBoundaryList
                                    # ['%s' %i for i in primaryBoundaryList]
                                    primaryList = primaryBoundaryList
                                elif content == 'institution':

                                    # ['%s' %i for i in preSchList]
                                    preList = preSchList
                                    # ['%s' %i for i in primarySchList]
                                    primaryList = primarySchList
                                elif content == 'staff':
                                    preStaffList = \
                                        Staff.objects.filter(
                                            institution__id__in=preSchList,
                                            institution__boundary__boundary_type__id=2).values_list('id',
                                                                                                    flat=True)
                                    primaryStaffList = \
                                        Staff.objects.filter(
                                            institution__id__in=primarySchList,
                                    ...
                                    preStList = \
                                        Student_StudentGroupRelation.objects.filter(
                                            student_group__id__in=preSGList).values_list(
                                            'student',
                                            flat=True)

                                    primaryStList = \
                                        Student_StudentGroupRelation.objects.filter(
                                            student_group__id__in=primarySGList).values_list(
                                            'student',
                                            flat=True)
                    ...
                    print '%s.csv file has been created in %s/logFiles directory' \
                        % (fileName, cwd)
                                    ...
                                    # if arguments are not proper raises an
                                    # command error.
                    ...
                    raise CommandError('Date Should be in dd/mm/yyyy format.\n'
                            ...
                            # if arguments are not passed raises an command
                            # error.
