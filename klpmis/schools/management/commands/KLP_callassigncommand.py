...
class Command(BaseCommand):
    #option_list = BaseCommand.option_list + (make_option('--user',
        # dest='user', type='string',help='User running the command'),)
    ...
    def handle(self, *args, **options):
        ...
        # get permission type
        message = "A mail will be sent to team@klp.org.in as soon as all\
        ...
        if bound_cat == 'district':
            # if boundary category is district query
                # for institutions under sub
            # boundaries
            for bound in bound_list:
                inst_list = Institution.objects.filter(
                    boundary__parent__id=bound,
                ...
                call_command(
                    ...
                    interactive=False,
                    inst_list=inst_list,
                    deUserList=deUserList,
                    permissions=permissions,
                    permissionType=permissionType,
                    assessmentId=assessmentId,
                    assessmentPerm=assessmentPerm)

                #asmIdList = assignPermission(inst_list, deUserList,
                    # permissions, permissionType, assessmentId,
                    # assessmentPerm)
                # assignedAsmIds.extend(asmIdList)
        elif bound_cat in ['block', 'project']:
            # if boundary category is district query
            # for institutions under sub boundaries
            for bound in bound_list:
                inst_list = Institution.objects.filter(
                    boundary__id=bound,
                    active=2).values_list('id',
                ...
                #asmIdList = assignPermission(inst_list, deUserList,
                    ...
                    # assessmentId, assessmentPerm)
                # assignedAsmIds.extend(asmIdList)
        if assessmentPerm:
            assignedAsmIds = list(set(assignedAsmIds))
            # get length of assessment ids to show count of assigned assessment
            # objects to user
            asmCount = len(assignedAsmIds)
            respDict[
                'respMsg'] = message
        # 'Assigned Permissions successfully for %s Institutions
        # and %s Assessments Assigned successfully' %(count, asmCount)
        else:
            respDict[
                'respMsg'] = message
...
def Tosendmailteam(inst_list, deUserList, permissions,
    ...
    Permissions : %s \n Permission Type :%s \n \
    Assessment Id List : %s \n Assessment Permissions : %s " % (
        inst_liststr, deUserList, permissions,
        permissionType, assessmentId, assessmentPerm)
