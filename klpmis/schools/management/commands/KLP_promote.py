...
def exec_bulk_insert(sqlQuery):
    ...
    p = subprocess.Popen(
        "psql -fklpQuery.txt klpmis klpmis",
...
def promote_tenth_students(academic_id):
    promoteStudentGroupSql = "begin;insert into schools_student_studentgrouprelation_3( student_id,student_group_id, academic_id, active) select student_id,student_group_id, academic_id,3 from schools_student_studentgrouprelation_2 where academic_id = %s and student_group_id in (select id from schools_studentgroup where name='10');commit;" % (
        academic_id)
    removeStudentGroupSql = "begin;delete from schools_student_studentgrouprelation_2 where academic_id = %s and student_group_id in (select id from schools_studentgroup where name='10');commit;" % (
        academic_id)
    ...
    removeStudentSql = "begin;delete from schools_student_2 where id in  (select student_id from schools_student_studentgrouprelation_2 where academic_id=%s and student_group_id in( select id from schools_studentgroup where name ='10'));commit;" % (
        academic_id)
...
def deactivate_current_year_student_sg_association(academic_id):
    deactivateStudentGroupSql = "begin;insert into schools_student_studentgrouprelation_1( student_id,student_group_id, academic_id, active) select student_id,student_group_id, academic_id,1 from schools_student_studentgrouprelation_2 where academic_id = %s and student_group_id in (select id from schools_studentgroup where name in ('1','2','3','4','5','6','7','8','9'));commit;" % (
        academic_id)
    removeStudentGroupSql = "begin;delete from schools_student_studentgrouprelation_2 where academic_id = %s and student_group_id in (select id from schools_studentgroup where name in ('1','2','3','4','5','6','7','8','9'));commit;" % (
        academic_id)
...
def validate_db(db1, currentAcademicYear, nextAcademicYear):
    err_txt = ""
    invalid_nextyear_st_grp_rel_sql = "select * from \
        schools_student_studentgrouprelation_2 \
        where academic_id=%s limit 1" % nextAcademicYear
    result = run_query(db1, invalid_nextyear_st_grp_rel_sql)
    if result:
        err_txt += "[ERROR] studentgroup relation \
            table already has next academic year's records;"
    invalid_curyear_st_grp_rel_sql = "select * from \
        schools_student_studentgrouprelation \
        where academic_id=%s and active in(1,3) limit 1" % (
        currentAcademicYear)
    result = run_query(db1, invalid_curyear_st_grp_rel_sql)
    if result:
        err_txt += "[ERROR] studentgroup relation table already \
            has current academic year's inactive/ promoted records;"
    return err_txt


def move_class_sql(cur_studentgroup_id, next_studentgroup_id,
                   currentAcademicId, nextAcademicId, sgname=10):

    if sgname < 10:

        createNextClassEntriesSql = "begin;insert into \
            schools_student_studentgrouprelation_2(student_id, \
                    student_group_id, \
                    academic_id, active) select  student_id,%s, %s, \
            2 from schools_student_studentgrouprelation_1 \
            where academic_id= %s and student_group_id =%s;commit;" % (
            next_studentgroup_id, nextAcademicId,
            currentAcademicId, cur_studentgroup_id)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        inslist = []
        currentAcademicYearSql = "select id,name from \
            schools_academic_year where active=1"
        ...
        nextAcademicYearSql = "select id,name from \
            schools_academic_year where name='%s'" % (
            nextAcademicYear)
        ...
        if not validation_errors:
            ...
            query1 = """select distinct institution_id as id from \
                schools_studentgroup  where id in  \
                (select distinct student_group_id from \
                    schools_Student_StudentGroupRelation_1 \
                    where academic_id=%s and student_group_id \
                    in (select id from schools_studentgroup where \
                        active=2 and institution_id in (select id from  \
                            schools_institution where cat_id \
                            not in (10,11,12) and active=2) and \
                active=2 and group_type='Class')) """ % (currentAcademicId)
            print query1
            ...
            for insobj in inst_list:
                ...
                if 1:
                    instid = insobj[0]
                    insquery = """select id, name from \
                        schools_institution where id=%s""" % (instid)
                    ...
                    query2 = """select distinct student_group_id \
                        as id from schools_student_studentgrouprelation_1 \
                        where academic_id =%s and student_group_id in \
                        (select id from schools_studentgroup where active=2 \
                            and institution_id =%s and group_type='%s' \
                            order by id)""" % (currentAcademicId, instituteId,
                                               'Class')
                    ...
                    for sg in sglist:
                        ...
                        try:
                            ...
                            sgSql = (
                                "select id,name,section from \
                                    schools_studentgroup where id =%s" %
                                (sgid))
                            sgRec = run_query(db, sgSql)
                            sgname = sgRec[0][1]
                            sgsection = sgRec[0][2]
                            # if it fails, then its not a valide class name, so
                            # skip and proceed
                            int_sg = int(sgname)
                            ...
                            if int_sg < 10:
                                nxtGroup = int_sg + 1
                            nextSgRec = run_query(
                                db, "select id,active from schools_studentgroup\
                                    where institution_id=%s and name='%s' and \
                                    section='%s'" %
                                (instituteId, nxtGroup, sgsection))
                            if not nextSgRec:
                                run_query(
                                    db, "begin;insert into \
                                        schools_studentgroup\
                                        (institution_id, name, section, \
                                            active, group_type) \
                                    values(%s, '%s', '%s',2,'Class') ;commit;" %
                                    (instituteId, nxtGroup, sgsection))
                                nextSgRec = run_query(
                                    db, "select id,name, section from \
                                        schools_studentgroup where \
                                        institution_id =%s and name='%s' and  \
                                        section='%s' and active=2 and \
                                        group_type='%s' " %
                                    (instituteId, nxtGroup, sgsection, 'Class'))
                            elif nextSgRec[0][1] != 2:
                                updateSgRec = run_query(
                                    db, "begin;update schools_studentgroup \
                                    set active=2 where institution_id=%s and\
                                     name='%s' and section='%s';commit;" %
                                    (instituteId, nxtGroup, sgsection))
