...
def student_update_status(contentName, schoolStudentsList):
    created = validFullHistoryRecords.filter(
        request__user_pk=user.id,
        content_type__id=contentTypeIds[contentName],
        object_id__in=schoolStudentsList,
        action='C').count()
    modified = validFullHistoryRecords.filter(
        request__user_pk=user.id,
        content_type__id=contentTypeIds[contentName],
        object_id__in=schoolStudentsList,
        action='U').exclude(_data__contains='"active": [2,').count()
    deleted = validFullHistoryRecords.filter(
        request__user_pk=user.id,
        content_type__id=contentTypeIds[contentName],
        object_id__in=schoolStudentsList,
        action='U', _data__contains='"active": [2,').count()

    return {'created': created, 'modified': modified, 'deleted': deleted}


def user_boundary_update(contentTypeName):
    strList = validFullHistoryRecords.filter(
        request__user_pk=user.id,
        content_type__id=contentTypeIds[contentTypeName]).only('object_id').values_list('object_id',
                                                                                        flat=True)
...
class Command(BaseCommand):
    ...
    A csv file will be created as output in a subfolder called logfiles
    in current folder.'''
        ...
        if 1:  # try:
            ...
            if fileName and start_date and end_date:
                if 1:  # try:
                    ...
                    cwd = os.getcwd()
                    path = '%s/logFiles/' % cwd
                    # if dir not exists creates directory with name logfiles in
                    # cwd.
                    ...
                    validUserIds = \
                        validFullHistoryRecords.only('request__user_pk'
                                                     ).distinct().values_list('request__user_pk', flat=True)
                    ...
                    answersUpdatedStrList = \
                        validFullHistoryRecords.filter(
                    ...
                    for (slNo, user) in enumerate(users):
                        ...
                        #  C => Content created
                        #  U + data contains "active: '[2,
                        # => Content modified and status changed
                        # from active-state to non-active states(deleted!)
                        #  U + data DOES-NOT contain "active: '[,
                        # => content updated and is still in active state!
                        #
                        #

                        boundariesUpdatedByUser = user_boundary_update(
                            'boundary')

                        if boundariesUpdatedByUser:
                            preSchoolBoundaries = \
                                Boundary.objects.filter(
                                    id__in=boundariesUpdatedByUser,
                            ...
                            if preSchoolBoundaries:
                                ...
                                preboundaryDict = student_update_status(
                                    'boundary', preSchoolBoundaries)
                                pre_boundary_created = preboundaryDict[
                                    'created']
                                pre_boundary_modified = preboundaryDict[
                                    'modified']
                                pre_boundary_deleted = preboundaryDict[
                                    'deleted']
                            ...
                            if primarySchoolBoundaries:
                                primarySchoolBoundaries = ['%s' % i
                                                           for i in
                                                           primarySchoolBoundaries]
                                primaryboundaryDict = student_update_status(
                                    'boundary', primarySchoolBoundaries)
                                primary_boundary_created = primaryboundaryDict[
                                    'created']
                                primary_boundary_modified = primaryboundaryDict[
                                    'modified']
                                primary_boundary_deleted = primaryboundaryDict[
                                    'deleted']
                        ...
                        if schoolsUpdatedByUser:
                            preSchools = [str(school) for school in
                                          Institution.objects.filter(
                                              id__in=schoolsUpdatedByUser,
                                              boundary__boundary_type__id=2).only('id'
                                                                                  ).values_list('id', flat=True)]
                            if preSchools:
                                schoolDict2 = student_update_status(
                                    'institution', preSchools)
                            ...
                            primarySchools = [str(school) for school in
                                              Institution.objects.filter(
                                                  ...
                                                  boundary__boundary_type__id=1).only('id'
                                                                                                    ...
                                                                                                    flat=True)]
                            if primarySchools:
                                schoolDict1 = student_update_status(
                                    'institution', primarySchools)
                        ...
                        if studentsUpdatedByUser:
                            preSchoolStudents = [str(student)
                                                 for student in
                                                 Student_StudentGroupRelation.objects.filter(
                                                     student__id__in=studentsUpdatedByUser,
                            ...
                            if preSchoolStudents:
                                preSchoolStudents = ['%s' % i for i in
                                                     preSchoolStudents]
                                studentDict2 = student_update_status(
                                    'student', preSchoolStudents)
                            ...
                            if primarySchoolStudents:
                                ...
                                studentDict = student_update_status(
                                    'student', primarySchoolStudents)
                        ...
                        if teachersUpdatedByUser:
                            ...
                            primarySchoolTeachers = [str(teacher)
                                                     for teacher in
                                                     Staff.objects.filter(
                                                         id__in=studentsUpdatedByUser,
                                                         institution__boundary__boundary_type__id=1)]
                            if primarySchoolTeachers:
                                studentDict3 = student_update_status(
                                    'staff', primarySchoolTeachers)
                                primary_teacher_created = studentDict3[
                                    'created']
                                primary_teacher_modified = studentDict3[
                                    'modified']
                                primary_teacher_deleted = studentDict3[
                                    'deleted']
                        ...
                        if answersUpdatedByUser:
                            for assessmentId in \
                                ...
                                answerEnteredIds = \
                                    validFullHistoryRecords.filter(
                                        ...
                                        action='C').only('object_id'
                                                                       ...
                                                                       flat=True)
                                # ids of answer records that i created
                                ...
                                # how many others records I verified by double entry
                                ...
                                answer_rectified = \
                                    validFullHistoryRecords.filter(
                                        ...
                                        action='U',
                                        _data__icontains='answer'
                                    ).exclude(object_id__in=answerEnteredIds).count()
                            # how many others wrong answers I corrected/ fixed by
                            # double entry
                                assessmentsModified[str(assessmentId)
                                                    + '_rectified'] = \
                                    answer_rectified
                                ...
                                answer_wrong = \
                                    validFullHistoryRecords.filter(content_type__id=contentTypeIds['answer'
                                                                   ...
                                                                   ', ',
                                                                   _data__icontains='answer',
                                                                   object_id__in=answersUpdatedForGivenAssessment).exclude(request__user_pk=user.id).count()
                            # How many of my answers were wrong...;
                            # showing all recored, which were created by me, but now updated by others.
                                assessmentsModified[str(assessmentId)
                                                    + '_wrong'] = answer_wrong
                    ...
                    print '%s.csv file has been created in %s/logFiles directory' \
                      ...
                      # except IndexError: # if arguments are not proper raises
                      # an command error.
