...
def KLP_StudentGroup_Answer_Entry(request, studentgroup_id, programme_id, assessment_id):
    ...
    ordercounter = settings.NUM_OF_FLEXI_ANSWER_FORM_RECORDS
    ...
    if AssObj.flexi_assessment:
        ansflexObj = list(set(Answer.objects.filter(question__in=question_list,
                                                    object_id__in=studIdList).distinct().order_by('id').values_list('flexi_data',flat=True)))
        ansflexObj.sort()
    else:
        ansflexObj = Answer.objects.filter(question__in=question_list,
                                           object_id__in = studIdList).order_by('id').values_list('flexi_data',flat=True)
    if ansflexObj:
    ...
    if not AssObj.flexi_assessment :
        ...
        if AssObj.primary_field_type==4:
            ...
            lookupfieldsnames = dict(lookupfields.values_list('id',
                                                              'name'))
